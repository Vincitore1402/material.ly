...
config = getConfig() # передаем модуль с конфигом для бд с MySQL Server

app = Flask(__name__) # создаем обьект класса Фласк , передавая туда имя модуля

app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:root@localhost/rloveshhenko$mydbtest'
db  =  SQLAlchemy ( app )
...
class ModelMechanicalProperties(CommonModel):
    form_columns = ['main_info_id', 'sortament', 'razmer', 'napr', 'sigma_b', 'sigma_t', 'delta_5', 'psi', 'KCU', 'termo_obrab']
...
MMP = ModelMechanicalProperties(MechanicalProperties, db.session, category= 'OtherTable', endpoint = 'MechanicalProperties')
...
if __name__ == '__main__':
    config = getConfig()
    app.secret_key = config.SECRET_KEY# задаем секретный ключ для работы с сессиями
    app.run(debug=True)
