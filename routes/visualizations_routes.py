...
from utils.common_utils import load_data_from_file
...
def yield_strength():
  try:
    ...
    result = load_data_from_file('chartData.txt')
    data = {
      'Silicium (Si)': result['data2'],
      'Carboneum (С)': result['data1']
    }

    graph_data = pygalService.create_xy_chart(data)

    return render_template('pygal.html', graph_data=graph_data,
                           title='Зависимость предела текучести для остаточной деформации от концентрации химического элемента для сталей:')
...
def manifold_learning():
  ...
  data = load_data_from_file('manifPygal.txt')
  for item in data:
